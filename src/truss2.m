%2D Truss Analysis
% Input Data
NN=xlsread('truss2.xls','Sheet2','F3');
Coor=xlsread('truss2.xls','Sheet2','C9:E100');
for j=1:NN
    i=Coor(j,1);
    X(i)=Coor(j,2);
    Y(i)=Coor(j,3);
...
NE=xlsread('truss2.xls','Sheet2','F4');
Pro=xlsread('truss2.xls','Sheet2','H9:L100');
for j=1:NE
    i=Pro(j,1);
    E(i)=Pro(j,2);
    A(i)=Pro(j,3);
...
NC=xlsread('truss2.xls','Sheet2','R3');
Cons=xlsread('truss2.xls','Sheet2','P9:Q100');

NF=xlsread('truss2.xls','Sheet2','Y3');
Forc=xlsread('truss2.xls','Sheet2','V9:X100');


% Figure of Truss without Deformation
for i=1:NE
    xx=[X(Dir(i,2)),X(Dir(i,3))];
    yy=[Y(Dir(i,2)),Y(Dir(i,3))];
    axis([-20 220 -20 120])
    line(xx,yy,'Color','b')
